...
struct SL_TIME_CONV G_Start_Time = {
	0,	
	{0, 0},	
};
			...
			else
			{
				...
				SlLock(f->RecvLock);
				{
					while (true)
					{
						...
						SlCopy(SL_TIMESTAMP_OF_PACKET(buf, num), &q->TimeStamp, sizeof(SL_TIMEVAL));
						num++;
						SlFree(q);
					}
				}
									 ...
									 ULONG receive_flags)
{
	...
	SlLockList(a->Device->FileList);
	{
		if (a->Halt == false && SL_LIST_NUM(a->Device->FileList) != 0)
		{
			...
			while (nbl != NULL)
			{
				...
				while (nb != NULL)
				{
					...
					GET_TIME(&time_stamp, &G_Start_Time);
						...
						if (ptr != NULL)
						{
							...
							for (i = 0;i < SL_LIST_NUM(a->Device->FileList);i++)
							{
								...
								if (f->NumRecvPackets < SL_MAX_PACKET_QUEUED)
								{
									...
									SlCopy(&q->TimeStamp, &time_stamp, sizeof(SL_TIMEVAL));
...
VOID TIME_SYNCHRONIZE(struct SL_TIME_CONV *data)
{
	if (data->reference != 0)
	{
		return;
...
void TIME_DESYNCHRONIZE(struct SL_TIME_CONV *data)
{
	data->reference = 0;
	//	data->start.tv_sec = 0;
	//	data->start.tv_usec = 0;
}
void SynchronizeOnCpu(struct SL_TIMEVAL *start)
{
...
void GetTimeKQPC(struct SL_TIMEVAL *dst, struct SL_TIME_CONV *data)
{
	LARGE_INTEGER PTime, TimeFreq;
	LONG tmp;
	static struct SL_TIMEVAL old_ts={0,0};
...
void GET_TIME(struct SL_TIMEVAL *dst, struct SL_TIME_CONV *data)
{
